...
import { generateExcelVictory } from "@/lib/excelFunc";
import { columnsSetsApplicationVictory } from "./myFilter";
...
export function TableVictory ({ applications }: { applications: constructionApplications[] }) {
  ...
  const columns = columnsSetsApplicationVictory[0]; // Выбор набора столбцов по индексу
      ...
      <div className="flex md:items-start w-full bg-white mt-5 text-black">
        <div className="flex flex-col items-center md:items-start w-full h-full md:w-[300px] gap-5 p-4">
          ...
          <button
            ...
            onClick={() => generateExcelVictory(applicationsExcel)}
          >
            Скачать
          </button>
