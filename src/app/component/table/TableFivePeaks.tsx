...
import { generateExcelFivePeaks } from "@/lib/excelFunc";
import { columnsSetsApplicationFivePeaks } from "./myFilter";
...
export function TableFivePeaks({
  applications,
}: {
  applications: constructionApplications[];
}) {
  const [applicationsArr, setApplicationsArr] =
    useState<constructionApplications[]>(applications);
  const [applicationsExcel, setApplicationsExcel] =
    useState<constructionApplications[]>(applications);
  ...
  const columns = columnsSetsApplicationFivePeaks[0]; // Выбор набора столбцов по индексу
      ...
      <div className="flex md:items-start w-full bg-white mt-5 text-black">
        <div className="flex flex-col items-center md:items-start w-full h-full md:w-[300px] gap-5 p-4">
          ...
          <button
            ...
            onClick={() => generateExcelFivePeaks(applicationsExcel)}
          >
            Скачать
          </button>
