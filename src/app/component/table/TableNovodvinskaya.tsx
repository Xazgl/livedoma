...
import { generateExcelNovodvinskaya } from "@/lib/excelFunc";
import {  columnsSetsApplicationNovodvinskaya } from "./myFilter";
...
export function TableNovodvinskaya ({ applications }: { applications: constructionApplications[] }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/appnovodvinskaya?' + params)
      .then(res => res.json())
      .then(el => {
  ...
  const columns = columnsSetsApplicationNovodvinskaya[0]; // Выбор набора столбцов по индексу
      ...
      <div className="flex md:items-start w-full bg-white mt-5 text-black">
        <div className="flex flex-col items-center md:items-start w-full h-full md:w-[300px] gap-5 p-4">
          ...
          <button
            ...
            onClick={() => generateExcelNovodvinskaya(applicationsExcel)}
          >
            Скачать
          </button>
