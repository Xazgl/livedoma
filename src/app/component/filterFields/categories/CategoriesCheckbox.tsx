...
export function CategoriesCheckbox({ filteblackProps,currentFilter, setCurrentFilter }: Props) {


    return <Accordion defaultExpanded={true} sx={{ width: '100%' }}>
        <AccordionSummary
            expandIcon={<ExpandMoreIcon />}
            aria-controls="panel1a-content"
            id="panel1a-header"
        >
            <Typography sx={{ fontSize: '14px' }}><HouseIcon/> Категория </Typography>
        </AccordionSummary>
        <AccordionDetails>
                                ...
                                return {
                                    ...
                                    category: prevFilterState?.category
                                        ? prevFilterState.category.includes(category)
                                            ? prevFilterState.category.filter(el => el !== category)
